...
async function selectProject(id) {
    ...
    try {
        ...
        startNeuroPolling(id);
    } catch (err) {
        console.error("Failed to load project details", err);
    }
}

let neuroPollInterval = null;
function startNeuroPolling(projectId) {
    if (neuroPollInterval) clearInterval(neuroPollInterval);
    updateNeuroStatus(projectId);
    neuroPollInterval = setInterval(() => updateNeuroStatus(projectId), 2000);
}

async function updateNeuroStatus(projectId) {
    try {
        const response = await fetch(`/api/projects/${projectId}/neuro-status`);
        const status = await response.json();
        renderNeuroHub(status);
    } catch (err) {
        console.error("Neuro status poll failed", err);
    }
}

function renderNeuroHub(data) {
    const container = document.getElementById('neuro-hub-container');
    if (!container) return;

    const v = data.vagus || {};
    const l = data.ligand?.vector || {};
    const s = data.saga || {};

    const vagusStatus = v.status || "UNKNOWN";
    const vagusClass = vagusStatus.toLowerCase();

    container.innerHTML = `
        <div class="neuro-hub">
            <p class="section-title">Neuro-Systemic Hub</p>
            <div class="neuro-grid">
                <div class="neuro-card">
                    <h4>Vital Signs (VAGUS)</h4>
                    <div class="vital-status ${vagusClass}">${vagusStatus}</div>
                    <div class="vital-metrics">
                        CPU: ${v.metrics?.cpu_pct?.toFixed(1) || '0'}% | 
                        RAM: ${v.metrics?.memory_pct?.toFixed(1) || '0'}%
                    </div>
                </div>
                <div class="neuro-card">
                    <h4>Identity (SAGA)</h4>
                    <div class="saga-identity">"${s.current_narrative || 'Initializing identity...'}"</div>
                    ${s.theme_signal ? `<div class="theme-badge">${s.theme_signal.theme} (${(s.theme_signal.confidence * 100).toFixed(0)}%)</div>` : ''}
                </div>
            </div>
            <div class="neuro-card" style="margin-top: 12px; grid-column: span 2;">
                <h4>LIGAND Disposition</h4>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                    ${Object.entries(l).map(([chem, val]) => `
                        <div class="chemical-row">
                            <div class="chemical-header">
                                <span>${chem.toUpperCase()}</span>
                                <span>${(val * 100).toFixed(0)}%</span>
                            </div>
                            <div class="chemical-bar">
                                <div class="chemical-fill" style="width: ${val * 100}%; background: ${getChemColor(chem)}"></div>
                            </div>
                        </div>
                    `).join('')}
                </div>
...
function getChemColor(chem) {
    ...
    return colors[chem.toLowerCase()] || '#4f46e5';
}
...
function renderProjectDetails(data) {
    ...
    elProjectDetails.innerHTML = `
        ...
        <div class="details-body">
            <div id="neuro-hub-container"></div>
            <div class="section">
