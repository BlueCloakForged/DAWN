...
@app.get("/api/projects/{project_id}/neuro-status")
async def get_neuro_status(project_id: str):
    """
    Fetch the complete neuro-systemic state for a project.
    Combines VAGUS (Vitals), LIGAND (Disposition), and SAGA (Identity).
    """
    ...
    status = {
        "vagus": {"status": "UNKNOWN", "metrics": {}},
        "ligand": {"vector": {}},
        "saga": {"traits": {}, "theme": "NEUTRAL"}
    }

    # 1. VAGUS (Vitals)
    vagus_path = project_dir / "artifacts" / "vagus.monitor_resources" / "vagus.vital_signs.json"
    if vagus_path.exists():
        with open(vagus_path, "r") as f:
            status["vagus"] = json.load(f)

    # 2. LIGAND (Disposition)
    ligand_path = project_dir / "ligand.pool.json"
    if ligand_path.exists():
        with open(ligand_path, "r") as f:
            status["ligand"] = json.load(f)

    # 3. SAGA (Identity)
    saga_path = project_dir / "saga.narrative_identity.json"
    if saga_path.exists():
        with open(saga_path, "r") as f:
            status["saga"] = json.load(f)
    
    # 4. Global Theme (Theme signal)
    theme_path = project_dir / "saga.global_theme.json"
    if theme_path.exists():
        with open(theme_path, "r") as f:
            status["saga"]["theme_signal"] = json.load(f)
...
# --- AIPAM Forensic Endpoints ---

@app.get("/api/projects/{project_id}/forensic")
async def get_forensic_view(project_id: str):
    """
    AIPAM forensic dashboard view.
    Returns analysis metrics, severity breakdown, findings summary,
    and detection rules status.
    """
    project_dir = PROJECTS_DIR / project_id
    if not project_dir.exists():
        raise HTTPException(status_code=404, detail="Project not found")
    ...
    metrics_path = project_dir / "artifacts" / "analyze.forensic_cot" / "analysis_metrics.json"
    if metrics_path.exists():
        with open(metrics_path, "r") as f:
            result["metrics"] = json.load(f)
        result["has_aipam_data"] = True

    # Load reviewed findings for severity breakdown
    reviewed_path = project_dir / "artifacts" / "hitl.findings_review" / "findings_reviewed.json"
    if reviewed_path.exists():
        ...
        for f_item in findings:
            ...
            if f_item.get("analyst_status") == "confirmed":
                confirmed += 1
            elif f_item.get("analyst_status") == "false_positive":
                false_pos += 1
        ...
        result["findings_summary"] = {
            ...
            "review_mode": reviewed.get("review_mode", "unknown"),
        }
        result["has_aipam_data"] = True

    # Load detection rules status
    suricata_path = project_dir / "artifacts" / "export.detection_rules" / "suricata_rules.rules"
    sigma_path = project_dir / "artifacts" / "export.detection_rules" / "sigma_rules.yaml"
    if suricata_path.exists():
        with open(suricata_path, "r") as f:
            text = f.read()
        rule_count = sum(1 for line in text.split("\n")
                         if line.strip() and not line.strip().startswith("#"))
        result["detection_rules"]["suricata"] = rule_count
    if sigma_path.exists():
        with open(sigma_path, "r") as f:
            sigma_data = json.load(f)
        result["detection_rules"]["sigma"] = sigma_data.get("total_rules", 0)

    # Trust receipt
    receipt_path = project_dir / "artifacts" / "quality.release_verifier" / "trust_receipt.md"
    if receipt_path.exists():
        with open(receipt_path, "r") as f:
            result["trust_receipt"] = f.read()

    return result


@app.get("/api/projects/{project_id}/campaigns")
async def get_campaign_correlations(project_id: str):
    """
    Campaign correlation groups for cross-project intelligence.
    """
    project_dir = PROJECTS_DIR / project_id
    if not project_dir.exists():
        raise HTTPException(status_code=404, detail="Project not found")

    corr_path = project_dir / "artifacts" / "correlate.campaign" / "campaign_correlation.json"
    if not corr_path.exists():
        return {"has_correlations": False, "groups": []}
    ...
    return {
        ...
        "total_findings_scanned": data.get("total_findings_scanned", 0),
        "groups": data.get("correlation_groups", []),
        "summary": data.get("summary", {}),
    }


@app.get("/api/projects/{project_id}/evidence-pack")
async def download_evidence_pack(project_id: str):
    """
    Download a ZIP archive of artifacts/ + ledger/ for analyst handoff.
    """
    import zipfile
    import tempfile
    ...
    tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".zip",
                                      prefix=f"evidence_{project_id}_")
    zip_path = tmp.name
    tmp.close()
    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        if artifacts_dir.exists():
            for fp in artifacts_dir.rglob("*"):
                if fp.is_file():
                    arcname = f"artifacts/{fp.relative_to(artifacts_dir)}"
                    zf.write(fp, arcname)

        # Add ledger/
        ledger_dir = project_dir / "ledger"
        if ledger_dir.exists():
            for fp in ledger_dir.rglob("*"):
                if fp.is_file():
                    arcname = f"ledger/{fp.relative_to(ledger_dir)}"
                    zf.write(fp, arcname)

        # Add project_index.json
        idx_path = project_dir / "project_index.json"
        if idx_path.exists():
            zf.write(idx_path, "project_index.json")

    return FileResponse(
        path=zip_path,
        filename=f"evidence_pack_{project_id}.zip",
